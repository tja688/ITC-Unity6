...
| `SignMiniGameFlowStateModel` | Model | 管理所有签约状态（当前客户、满意度、错误计数、金钱等） |
| `SignMiniGameClientConfigModel` | Model | 客户数据库管理 |
...
flowchart LR
  subgraph Model
    CSM[SignMiniGameFlowStateModel]
    CDM[SignMiniGameClientConfigModel]
  end
...
| `sign_minigame_ui`（新增） | DocumentReviewPanel, RuneTypingPanel, RuneVerifyPanel, StampPanel, SoulCollectPanel | 所有签约小游戏面板 |
...
| 面板频繁开关的资源 | Bundle 加载/卸载性能 | 签约期间预加载 `sign_minigame_ui` bundle |
