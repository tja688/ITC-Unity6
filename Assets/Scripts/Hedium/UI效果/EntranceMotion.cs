...
public class EntranceMotion : MonoBehaviour
{
    ...
    {
        var ease = ToDOTweenEase(inEasePreset);
        if (useLocalPosition)
        {
            var z = transform.localPosition.z;
            Vector3 target = new Vector3(endPosition.x, endPosition.y, endPosition.z != 0 ? endPosition.z : z);
            moveTween = transform.DOLocalMove(target, inDuration).SetEase(ease).SetUpdate(useUnscaledTimeForDelayAndFade)
        ...
        else
        {
            ...
            Vector3 target = new Vector3(endPosition.x, endPosition.y, endPosition.z != 0 ? endPosition.z : z);
            moveTween = transform.DOMove(target, inDuration).SetEase(ease).SetUpdate(useUnscaledTimeForDelayAndFade)
                .OnComplete(() => { IsAnimating = false; IsShown = true; onShown?.Invoke(); });
        }
        return moveTween;
    }
    ...
    {
        var ease = ToDOTweenEase(outEasePreset);
        if (useLocalPosition)
        {
            var z = transform.localPosition.z;
            Vector3 target = new Vector3(startPosition.x, startPosition.y, startPosition.z != 0 ? startPosition.z : z);
            moveTween = transform.DOLocalMove(target, outDuration).SetEase(ease).SetUpdate(useUnscaledTimeForDelayAndFade)
        ...
        else
        {
            var z = transform.position.z;
            Vector3 target = new Vector3(startPosition.x, startPosition.y, startPosition.z != 0 ? startPosition.z : z);
            moveTween = transform.DOMove(target, outDuration).SetEase(ease).SetUpdate(useUnscaledTimeForDelayAndFade)
                .OnComplete(() => { IsAnimating = false; IsShown = false; onHidden?.Invoke(); });
        }
        return moveTween;
    }
    ...
    static Ease ToDOTweenEase(EasePreset p)
    {
        switch (p)
        {
