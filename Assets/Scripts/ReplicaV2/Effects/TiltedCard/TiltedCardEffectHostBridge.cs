...
[ReplicaShowcase("Cards", "Tilted")]
[DisallowMultipleComponent]
[RequireComponent(typeof(RectTransform))]
public sealed class TiltedCardEffectHostBridge : MonoBehaviour, IReplicaShowcaseBridge
{
    [Header("Runtime")]
    ...
    [Header("Config")]
    [SerializeField] private TiltedCardEffectConfig mConfig;

    [Header("Default Model")]
    [SerializeField] private string mHint = "TiltedCard  |  Move cursor around the card";
    [SerializeField] private string mBadge = "HOVER TO TILT";

    private RectTransform mRootRect;
    private ReplicaPointerRelay mPointerRelay;
    private TiltedCardEffectController mController;
    private TiltedCardEffectModel mModel;

    private void Awake()
    {
        if (!TryGetComponent(out mRootRect))
        {
            Debug.LogError("TiltedCardEffectHostBridge requires RectTransform.", this);
            enabled = false;
            return;
        }
    ...
    {
        EnsureController();
        mModel = model != null ? model.Clone() : TiltedCardEffectModel.CreateDefault();
    ...
    public void UpdateBadge(string badge, bool animated = true)
    {
        EnsureModel();
        mModel.Badge = badge;
        mController.SetModel(mModel.Clone(), animated);
    ...
    private void EnsureController()
    {
        ...
        mController = new TiltedCardEffectController();
    ...
    private void EnsureModel()
    {
        ...
        mModel = new TiltedCardEffectModel
        {
            Hint = mHint,
            Badge = mBadge
        };
