...
[ReplicaShowcase("Sequence", "GlassIcons")]
[DisallowMultipleComponent]
[RequireComponent(typeof(RectTransform))]
public sealed class GlassIconsTiltEffectHostBridge : MonoBehaviour, IReplicaShowcaseBridge
{
    [Header("Runtime")]
    ...
    [Header("Config")]
    [SerializeField] private GlassIconsTiltEffectConfig mConfig;

    [Header("Default Model")]
    [SerializeField] private string mHint = "GlassIcons  |  Hover each icon tile";

    private RectTransform mRootRect;
    private ReplicaPointerRelay mPointerRelay;
    private GlassIconsTiltEffectController mController;
    private GlassIconsTiltEffectModel mModel;

    private void Awake()
    {
        if (!TryGetComponent(out mRootRect))
        {
            Debug.LogError("GlassIconsTiltEffectHostBridge requires RectTransform.", this);
            enabled = false;
            return;
        }
    ...
    {
        EnsureController();
        mModel = model != null ? model.Clone() : GlassIconsTiltEffectModel.CreateDefault();
    ...
    private void EnsureController()
    {
        ...
        mController = new GlassIconsTiltEffectController();
    ...
    private void EnsureModel()
    {
        ...
        mModel = GlassIconsTiltEffectModel.CreateDefault();
        mModel.Hint = mHint;
    }
}
