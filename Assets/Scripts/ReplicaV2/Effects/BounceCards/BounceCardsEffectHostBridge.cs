...
[ReplicaShowcase("Cards", "Bounce")]
[DisallowMultipleComponent]
[RequireComponent(typeof(RectTransform))]
public sealed class BounceCardsEffectHostBridge : MonoBehaviour, IReplicaShowcaseBridge
{
    [Header("Runtime")]
    ...
    [Header("Config")]
    [SerializeField] private BounceCardsEffectConfig mConfig;

    [Header("Default Model")]
    [SerializeField] private string mHint = "BounceCards  |  Hover a card to push siblings";

    private RectTransform mRootRect;
    private ReplicaPointerRelay mPointerRelay;
    private BounceCardsEffectController mController;
    private BounceCardsEffectModel mModel;

    private void Awake()
    {
        if (!TryGetComponent(out mRootRect))
        {
            Debug.LogError("BounceCardsEffectHostBridge requires RectTransform.", this);
            enabled = false;
            return;
        }

        mPointerRelay = ReplicaUIFactoryV2.EnsureComponent<ReplicaPointerRelay>(gameObject);
        mPointerRelay.SetFallbackToMouse(false);
    }
    ...
    {
        EnsureController();
        mModel = model != null ? model.Clone() : BounceCardsEffectModel.CreateDefault();
    ...
    private void EnsureController()
    {
        ...
        mController = new BounceCardsEffectController();
    ...
    private void EnsureModel()
    {
        ...
        mModel = new BounceCardsEffectModel
        {
            Hint = mHint
        };
    }
