...
[ReplicaShowcase("Sequence", "ElasticSlider")]
[DisallowMultipleComponent]
[RequireComponent(typeof(RectTransform))]
public sealed class ElasticOverflowSliderEffectHostBridge : MonoBehaviour, IReplicaShowcaseBridge
{
    [Header("Runtime")]
    ...
    [Header("Config")]
    [SerializeField] private ElasticOverflowSliderEffectConfig mConfig;

    [Header("Default Model")]
    [SerializeField] private string mHint = "ElasticSlider  |  Drag and over-pull both sides";
    [SerializeField] private float mDefaultValue = 50f;

    private RectTransform mRootRect;
    private ReplicaPointerRelay mPointerRelay;
    private ElasticOverflowSliderEffectController mController;
    private ElasticOverflowSliderEffectModel mModel;

    private void Awake()
    {
        if (!TryGetComponent(out mRootRect))
        {
            Debug.LogError("ElasticOverflowSliderEffectHostBridge requires RectTransform.", this);
            enabled = false;
            return;
        }
    ...
    {
        EnsureController();
        mModel = model != null ? model.Clone() : ElasticOverflowSliderEffectModel.CreateDefault();
        mController.SetModel(mModel.Clone(), animated);
    }

    public void UpdateValue(float value, bool animated = true)
    {
        EnsureModel();
        mModel.Value = value;
        mController.SetModel(mModel.Clone(), animated);
    ...
    private void EnsureController()
    {
        ...
        mController = new ElasticOverflowSliderEffectController();
    ...
    private void EnsureModel()
    {
        ...
        mModel = new ElasticOverflowSliderEffectModel
        {
            Hint = mHint,
            Value = mDefaultValue
        };
