...
* **项目默认文档位置**：项目默认文档目录是 `ITC Docs`，但这里只是入口目录，不代表其中所有文档都可以被当作事实源。
* **唯一事实源**：`ITC Docs/事实文档` 是本项目唯一允许被视为“事实源”的目录。除这个目录外，项目中其他任何文档、说明、注释、草稿、临时记录，都**不**构成唯一事实来源，也**不**作为事实维护对象。
* **禁止跨区找源**：在做文档核对、事实校验、事实回写或维护时，不要去项目其他目录里随意翻找“看起来像事实”的内容并据此改写。若某条事实不在 `ITC Docs/事实文档` 中，就视为当前没有被正式建模，不要自行补完。
* **开发前必核对事实文档**：开始开发前，必须先核对 `ITC Docs/事实文档` 中与任务相关的文档说明，只围绕该目录里的内容建立理解与定位。
* **事实回写要求**：凡是产生变更的事实，都必须落地回 `ITC Docs/事实文档` 内对应的事实源文档；新增则新增，删除则删除，变更则直接修改原事实，不允许在后续文档中打补丁式堆积，也不允许散落到其他目录维护。
* **更新文档时间**：任何 `ITC Docs/事实文档` 内的事实源文档被修改后，必须在最后更新文档更新时间。
* **事实治理目标**：始终确保 `ITC Docs/事实文档` 内的项目事实唯一、准确、不过度臃肿；其他目录不承担事实库职责。
* **开发计划例外**：头脑风暴与开发计划本身不受事实文档约束；如用户要求生成开发 plan，必须在开发计划路径下生成一份带标题的计划 `md` 文件，并使用相关模块文件夹进行外层包裹；如不存在则创建，如存在可承载的模块文件夹则优先放入其中。
* **专项建档前提**：除非开发者明确要求，不得主动根据开发内容新建事实文档；只有在明确指示下才允许进行专项建档。若当前任务尚未对某个逻辑进行建模，则无需为该逻辑补建事实文档。
